...
public class ImgProxy {
    private static ImgProxy instance = new ImgProxy();
     ...
     * @return the instance
     */
    public static synchronized ImgProxy getInstance() {
        return instance;
    }
     ...
     */
    public RequestManager with(Context aContext) {
     ...
     * 初始化图片加载请求管理者
     *
     * @param aActivity 图片依托的页面
     * @return request manager
     */
    public RequestManager with(Activity aActivity) {
        return Glide.with(aActivity);
    }
     ...
     * @return request manager
     */
    public RequestManager with(Fragment aFragment) {
...
sizeMultiplier(float sizeMultiplier). 在加载资源之前给Target大小设置系数。
    diskCacheStrategy(DiskCacheStrategy strategy).设置缓存策略。DiskCacheStrategy
    .SOURCE：缓存原始数据，DiskCacheStrategy.RESULT：缓存变换后的资源数据，DiskCacheStrategy
    .NONE：什么都不缓存，DiskCacheStrategy.ALL：缓存SOURC和RESULT。默认采用DiskCacheStrategy.RESULT策略，对于download
    only操作要使用DiskCacheStrategy.SOURCE。
    priority(Priority priority). 指定加载的优先级，优先级越高越优先加载，但不保证所有图片都按序加载。枚举Priority.IMMEDIATE，Priority
    .HIGH，Priority.NORMAL，Priority.LOW。默认为Priority.NORMAL。
    dontAnimate(). 移除所有的动画。
    ...
    isFromMemoryCache, boolean isFirstResource)和onException(Exception e, T model, Target&lt;R&gt;
     target, boolean isFirstResource)，但不要每次请求都使用新的监听器，要避免不必要的内存申请，可以使用单例进行统一的异常监听和处理。
